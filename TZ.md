...
### 4.13 Чек-лист внедрения (Этап 1, v1 shadow) — 2026-08 ✅ сделано (часть)
Первый shadow-коммит, намеренно уже полного §4.2-конвейера — см. обоснование
в самом коде (`lib/moderation/scoring.ts`, докстринг `runShadowScoring`).

- [x] `lib/moderation/scoring.ts`: `Signal`/`ScoreResult`/`Zone`, `collectSpamSignals`
      (порт детекторов `spam.ts` в форму «собрать все совпадения», не
      «вернуть первое»), `scoreSignals` (группа `link-risk` — только максимум,
      остальное аддитивно, модификатор репутации §4.5, зоны §4.7).
- [x] `MODERATION_V2=off|shadow|on` — читается в `moderationV2Mode()`,
      невалидное значение деградирует в `off`, не бросает исключение.
- [x] `runShadowScoring` вызывается из `bot.ts` сразу после реального
      вердикта, результат только логируется (`console.log`), никогда не
      влияет на `applyViolation`. Логи без raw text/username/userId (§11.4) —
      только `score`/`zone`/`signals[].name+weight`/`chatId`/`messageId`.
- [x] `ModerationVerdict.source` (новое необязательное поле, §4.9) — метка,
      какой детектор выдал старый вердикт, нужна чтобы `comparable` в логе
      отличал «скорер не смоделировал этот детектор» (flood/profanity/
      premium-ai/restricted-content/night-mode) от «скорер посчитал и не
      нашёл» — иначе процент расхождений старого/нового вердикта был бы
      бессмысленным (не калибровочная проблема, а «мы сравниваем разное»).
- [x] §4.11 защита от двойного счёта: CTA-фраза, уже учтённая в
      `forward_cta`/`link_cta`, не учитывается повторно в `mention_cta`/
      `cta_alone` — найдено и исправлено ревью до коммита.
- [x] Тесты: `scoring.test.ts` (18), проверяют сигналы, группу-максимум,
      модификатор репутации, зоны, регресс на двойной счёт CTA.

**Сознательно отложено на следующие коммиты** (см. докстринг `runShadowScoring`
и §11.3 для порядка):
- флуд-сигналы (`checkUserFlood`/`checkDuplicateFlood` — INCR-based чтения,
  вызов их из shadow-пути задвоил бы счётчик флуда для каждого сообщения);
- модификатор «новый аккаунт < 7 дней» (нужен `consumeNewMemberFlag` —
  одноразовый GET→DEL, уже потреблённый реальным пайплайном);
- DeepSeek как сигнал скоринга (§4.6) — переключение условия вызова
  `classifyWithDeepseek` трогает живой premium-путь, именно то, чего shadow-
  режим должен избегать в первую очередь;
- нечёткий дубль (§5) — модуль `fuzzy.ts`/окно дублей ещё не построены;
- `normalize.ts` НЕ расширен (см. §4.3) — новая версия с protected-token
  сепаратор-коллапсом была написана, но откачена: она меняла бы поведение
  уже живого `impersonation.ts` (тот же общий модуль), непроверенно и
  незапрошенно. Делать отдельно, с тестами на обоих потребителях, когда
  реально понадобится для §5.
- живое переключение `MODERATION_V2=on` на реальное действие — принимается
  как валидное значение, но пока идентично `shadow` по эффекту; wiring
...
#### Флуд-сигналы и модификатор «новый аккаунт» в shadow-скорере — 2026-08 ✅ сделано
Разблокировано атомарными G2/G5-фиксами выше (без них double-count/consume
чужого состояния был неизбежен).

- [x] `lib/moderation/flood.ts`: read-only `peekUserFloodCount`/
      `peekDuplicateFloodCount` (обычный `GET`, никогда не инкрементирует —
      не `checkUserFlood`/`checkDuplicateFlood`, которые задвоили бы счётчик
      реального пайплайна). Читают то же значение, что уже успел
      инкрементировать реальный пайплайн для этого сообщения — если он вообще
      его трогал (см. ниже про staleness).
- [x] `lib/moderation/flood.ts`: `isWithinNewMemberWindow` — новый read-only
      ключ `newmember:window:{chatId}:{userId}` (тот же 7-дневный TTL, что и
      одноразовый флаг), пишется вторым `SET` в `markNewMember` (один
      pipelined round-trip). Отдельный от одноразового `newmember:*` ключа
      намеренно: тот удаляется `consumeNewMemberFlag` на первом же сообщении,
      а §4.4/§4.5 требует проверяемый на каждом сообщении сигнал за всё
      7-дневное окно, не только на первом.
- [x] `scoreSignals` получил третий необязательный параметр `isNewAccount`
      (default `false`, обратная совместимость с 9 существующими тестами) —
      flat `+10` (§4.4), применяется **после** модификаторов репутации, а не
      до, иначе `score>=60`-флор (`Math.max(score, 21)`) мог бы молча
      поглотить его без эффекта.
- [x] `runShadowScoring`: `user_flood` (вес 50, порог `FLOOD_MAX_MESSAGES`) и
      `duplicate_flood` (вес 60, порог `DUPLICATE_MAX_COUNT`) добавляются как
      обычные `Signal` после `collectSpamSignals`, не внутри неё — Redis не
      должен попадать в чистую `collectSpamSignals` (18 тестов остаются
      дешёвыми/детерминированными). Четыре Redis-чтения (репутация + 2 peek +
      new-account) идут через `Promise.all`, не единый pipeline — они
      трогают ключи трёх разных модулей (`reputation.ts`, `flood.ts` x3), и
      экономия одного round-trip не стоит нарушения инкапсуляции.
- [x] **Известное смещение (задокументировано в коде, не исправлено):**
      `peekUserFloodCount`/`peekDuplicateFloodCount` читают счётчик реального
      пайплайна, который инкрементируется только если `antispam` включён,
      это не edit, и `detectSpam` уже не нашёл совпадения (см. `index.ts`).
      Значит именно на сообщениях, где старый спам-детектор уже сработал —
      ровно та популяция, где новый скорер вероятнее всего разойдётся со
      старым — флуд-сигнал систематически недосрабатывает (читает
      устаревший счётчик). Это смещает cross-tab из прошлого коммита в
      сторону "looser" на детект-совпадающих сообщениях (скорер выглядит
      мягче, чем он есть, именно там, где его сравнивают со старым спам-
      детектором) — не "stricter". Читать
      `shadow-report` с поправкой на это; не решается без отдельного
      независимого счётчика (сознательно не сделано — см. «peek вместо
      параллельных счётчиков» в решениях ниже).
- [x] **Известный backfill-разрыв:** участники, вступившие до этого деплоя,
      имеют только старый одноразовый ключ, `isWithinNewMemberWindow` будет
      `false` для них весь оставшийся срок (до 7 дней после деплоя) — не баг,
      просто первая неделя `shadow-report` недосчитает `+10`-модификатор.
- [x] Тесты: `scoring.test.ts` +11 (`isNewAccount` в `scoreSignals`, wiring
      `user_flood`/`duplicate_flood`/new-account в `runShadowScoring`, fail-open
      на отказе Redis), `flood.test.ts` +6 (`peek*`, `isWithinNewMemberWindow`,
      независимость от одноразового флага). `lib/db/fakeRedis.ts` получил
      `.pipeline()`/`.exists()` для этого.
...
| B2 MVP | Репутация (`rep:user`, снимает leniency у повторных нарушителей) + join-velocity как дешёвая версия `shared_group`-графа | `lib/moderation/reputation.ts` (новый), `lib/db/userGraph.ts` (новый), `lib/moderation/index.ts`, `lib/telegram/bot.ts` | ✅ сделано |
